...
import type { BriefingResult } from './claude';
...
export async function sendWeeklyNewsletter(env: Env): Promise<void> {
  ...
  const content = await env.DB.prepare(
    `SELECT date, briefing FROM daily_briefings WHERE date >= ? ORDER BY date DESC`,
  ).bind(weekAgo).all<{ date: string; briefing: string }>();
  ...
  const days = content.results.map((row) => ({
    date: row.date,
    data: JSON.parse(row.briefing) as BriefingResult,
  }));

  const today = new Date().toISOString().slice(0, 10);
  const subject = `${env.SITE_NAME} — Week of ${today}`;
  const baseHtml = buildWeeklyEmail(days, env);
  const workerHost = 'https://ai-pulse-worker.alexmerricks.workers.dev';
...
function buildWeeklyEmail(
  days: Array<{ date: string; data: BriefingResult }>,
  env: Env,
): string {
  const daySections = days.map((day) => {
    const stories = day.data.stories.map((item) => `
      <tr><td style="padding: 0 0 16px;">
        <a href="${esc(item.url)}" style="color: #c9a227; text-decoration: none; font-size: 14px; font-weight: 500;">${esc(item.title)}</a>
        <span style="color: #525252; font-size: 12px;"> [${item.source}] ${'●'.repeat(item.significance)}${'○'.repeat(5 - item.significance)}</span>
        <p style="color: #737373; font-size: 13px; margin: 4px 0 0; line-height: 1.5;">${esc(item.summary)}</p>
      </td></tr>
    `).join('');

    const papers = day.data.papers.length > 0 ? day.data.papers.map((paper) => `
      <tr><td style="padding: 2px 0;">
        <a href="${esc(paper.url)}" style="color: #d4d4d4; text-decoration: none; font-size: 13px;">${esc(paper.title)}</a>
        <span style="color: #525252; font-size: 11px;"> — ${esc(paper.authors)}</span>
        <p style="color: #737373; font-size: 12px; margin: 2px 0 8px;">${esc(paper.summary)}</p>
      </td></tr>
    `).join('') : '';
    ...
    return `
      ...
      <tr><td style="padding: 4px 32px 16px;">
        ...
        ${day.data.trend ? `<p style="color: #737373; font-size: 13px; margin: 8px 0 0; font-style: italic;">${esc(day.data.trend)}</p>` : ''}
      </td></tr>
      ...
      ${papers ? `
        <tr><td style="padding: 8px 32px 0;">
          <p style="font-family: monospace; font-size: 10px; color: #525252; letter-spacing: 1px; margin: 0 0 8px;">NOTABLE PAPERS</p>
          <table width="100%" cellpadding="0" cellspacing="0">${papers}</table>
        </td></tr>
      ` : ''}
